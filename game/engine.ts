...
export const SCENARIOS = [
  ...
  { id: 'oxygen', title: 'Oxygen processor failure', detail: 'Life-support integrity −60. Oxygen reserve −16.', building: 'lifeSupport' },
  { id: 'hull', title: 'Hull puncture', detail: 'Habitat integrity −60. Oxygen is escaping.', building: 'habitat' },
  { id: 'battery', title: 'Battery short', detail: 'Solar integrity −50. Power reserve −22. Stability −8.', building: 'solar' },
  { id: 'water', title: 'Recycler seizure', detail: 'Recycler integrity −60. Water reserve −20.', building: 'recycler' },
  { id: 'food', title: 'Greenhouse blight', detail: 'Greenhouse integrity −60. Food reserve −20.', building: 'greenhouse' },
] as const;
...
export type Evidence = {id:string;action:string;building?:Building;tick:number;doneTick?:number;status:'running'|'completed'|'rejected';cost:string;detail:string;beforeHealth?:number;responseId?:string};
export type ApiReceipt = {id:string;tick:number;requestedEffort:Effort;reportedEffort:string|null;reasoningTokens:number|null;totalTokens:number|null;status:'running'|'completed'};
export type Game = {
  evidence:Evidence[];apiReceipts:ApiReceipt[];disasters:{tick:number;id:ScenarioId}[];
...
export function inject(g: Game, id: ScenarioId, title?: string, replay=false): Result {
  const scenario=SCENARIOS.find(s=>s.id===id); if(!scenario)return {ok:false,message:'Unknown disaster.'};
  const error=replay?null:eligible(g,id);if(error)return {ok:false,message:error};
  g.lastChaos=g.tick;g.injections++;g.disasters.push({tick:g.tick,id});
  const damage=(b:Building,n:number)=>{g.health[b]=clamp(g.health[b]-n);g.revisions[b]++;};
  if(id==='oxygen'){damage('lifeSupport',60);g.resources.oxygen=clamp(g.resources.oxygen-16);}
  if(id==='hull')damage('habitat',60);
  if(id==='water'){damage('recycler',60);g.resources.water=clamp(g.resources.water-20);}
  if(id==='food'){damage('greenhouse',60);g.resources.food=clamp(g.resources.food-20);}
  if(id==='battery'){damage('solar',50);g.resources.power=clamp(g.resources.power-22);g.resources.stability=clamp(g.resources.stability-8);}
  else if(!g.crises.some(c=>c.id===id))g.crises.push({id,title:title||scenario.title,building:scenario.building,...(id==='dust'?{expires:g.tick+35}:{})});
...
export function action(g:Game, a:Action):Result {
  ...
  let result:Result;
  const b=a.args.building as Building;const beforeHealth=g.health[b];
  ...
  const changedSetting=(a.name==='set_power_mode'||a.name==='set_rationing');
  // Record engine receipts, not a narrative claim that a repair succeeded.
  if(result.ok||g.mode==='live'){
   const title:Record<string,string>={run_diagnostic:'Diagnostic',repair_module:'Repair',set_power_mode:'Power allocation',set_rationing:'Rationing',release_oxygen:'Emergency oxygen'};
   const receipt:Evidence={id:a.callId,action:title[a.name]||a.name,...(BUILDINGS.includes(b)?{building:b,beforeHealth}:{}),tick:g.tick,status:!result.ok?'rejected':result.jobId?'running':'completed',cost:!result.ok?'No supplies spent':a.name==='repair_module'?'1 part · 1 crew':a.name==='release_oxygen'?'1 oxygen canister':a.name==='run_diagnostic'?'1 scan slot':changedSetting?'No spare parts':'None',detail:result.message,...(!result.jobId?{doneTick:g.tick}:{})};
   g.evidence.push(receipt);if(g.evidence.length>80)g.evidence.shift();
  }
...
export function tick(g:Game):Completion[]{
  ...
  for(const j of g.jobs.filter(j=>j.due<=g.tick)){
    const beforeCompletion=g.health[j.building];let result:Result;
    if(j.type==='repair'){
    ...
    const receipt=g.evidence.find(e=>e.id===j.callId);
    if(receipt){receipt.status='completed';receipt.doneTick=g.tick;receipt.detail=j.type==='repair'?`${LABELS[j.building]} integrity ${beforeCompletion}% → ${g.health[j.building]}%. Completed in ${g.tick-receipt.tick}s.`:result.message;}
  ...
  const r=g.resources,delta=flows(g);
  ...
  g.oxygenHazard=r.oxygen<=15?g.oxygenHazard+1:0;g.supplyHazard=r.food<=5||r.water<=5?g.supplyHazard+1:0;
  if(g.oxygenHazard>=6){g.population=Math.max(0,g.population-1);g.oxygenHazard=0;log(g,'system','One colonist lost to oxygen deprivation.');}
  if(g.supplyHazard>=10){g.population=Math.max(0,g.population-1);g.supplyHazard=0;log(g,'system','One colonist lost to exhausted supplies.');}
  ...
  if(g.population===0||g.tick>=180){g.phase=g.population>=34?'won':'lost';g.jobs=[];g.operatorState='Mission complete';log(g,'system',g.phase==='won'?`Rescue arrived. ${g.population} colonists survived.`:`Mission lost. ${g.population} colonists remained at rescue.`);}
  return completed;
}
export function rehearse(g:Game){
  ...
  for(const b of ['lifeSupport','habitat','recycler','greenhouse','solar'] as Building[]){
    if(g.health[b]<80){
      ...
      if(g.parts>0&&!g.jobs.some(j=>j.type==='repair'&&j.building===b)&&g.jobs.filter(j=>j.type==='repair').length<2)act('repair_module',{building:b});
    }
  }
...
/** Exact per-second rules also power the visible trend and action evidence. */
export function flows(g:Game):Record<Resource,number>{
 const h=(b:Building)=>g.health[b]/100,r=g.resources,powered=r.power>5?1:.25;
 const solar=g.crises.some(c=>c.id==='dust')?.25:1;
 const leak=(g.crises.some(c=>c.id==='hull')?.70:0)+(g.health.habitat<50?.15:0);
 const ration=g.rationing?.7:1,focus=g.powerMode==='life_support';
 return {power:.8*h('solar')*solar-.65-(focus?.12:0),oxygen:.95*h('lifeSupport')*powered*(focus?1.5:1)-.85-leak,water:.32*h('recycler')*powered-.30*ration,food:(focus?0:.22*h('greenhouse')*powered)-.20*ration,stability:(['oxygen','power','water','food'].some(k=>r[k as Resource]<20)?-.15:.05)-(g.rationing?.05:0)};
}
/** Counterfactual inside these rules: replay the same accepted disasters with no operator actions. */
export function unattendedReplay(game:Game){
 const baseline=initialGame('rehearsal',game.seed);start(baseline);
 for(let t=0;t<=game.tick;t++){
  for(const event of game.disasters.filter(e=>e.tick===t))inject(baseline,event.id,undefined,true);
  if(t<game.tick)tick(baseline);
 }
 return {population:baseline.population,oxygen:baseline.resources.oxygen,power:baseline.resources.power};
}
